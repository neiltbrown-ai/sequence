...
- `content/reference/case-study-publish-protocol.md` — the end-to-end ship sequence for a case study: verify gate → git/deploy → **Airtable pipeline update**. Defines the publish convention for the "In Sequence — Case Studies" Airtable base (`appysSx11QTVyGHmM`): which Pipeline fields to set (Status→published, MDX Done, Images Sourced, Live URL) + the Update Log entry, with exact table/field IDs. Read + follow whenever case studies go live — the Airtable step is separate from the git deploy and easy to forget.
...
- **⚠ A `node_modules` symlink that forms a filesystem LOOP crashes `next dev` with a Turbopack panic.** Turbopack walks the whole project tree to build `src/app/globals.css` (Tailwind/PostCSS `track_glob`) and **follows symlinks**. If any symlink it reaches points back into an ancestor — the classic being a self-referential `tools/image-sourcer/node_modules/node_modules -> tools/image-sourcer/node_modules` (a `node_modules` linking to itself, usually from running `ln -s … node_modules node_modules` in the wrong cwd) — the glob recurses forever and you get: `FATAL: An unexpected Turbopack error occurred … Failed to write app endpoint /(public)/page` (the panic log names the looping symlink). It's a **whole-project** failure even though the bad link sits in a sibling tool dir, and it can lie dormant until a dev-server restart forces a fresh scan. **Diagnose:** read the panic-log path it prints, or `find tools -type l -name node_modules -exec sh -c 'echo "$1 -> $(readlink "$1")"' _ {} \;` to spot a self/loop link. **Fix:** `rm` the offending symlink, `rm -rf .next`, restart `next dev`. **Avoid:** a worktree nested under `tools/<x>/.claude/worktrees/…/tools/image-sourcer/` usually needs **no** `node_modules` symlink at all — Node resolves up the directory tree to the parent tool's real `node_modules` (verify with `node -e "require.resolve('express')"` before symlinking). Distinct from the SSD asset symlink `tools/remotion/public/cases` (media storage) — never confuse the two. Surfaced 2026-06-25.
