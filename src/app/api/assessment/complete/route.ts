...
export async function POST(request: Request) {
  ...
  // Use client-supplied answers (avoids autosave race condition), fall back to DB
  ...
  const answers: AssessmentAnswers = {
    ...
    discernment_questions: src.discernment_questions ?? assessment.discernment_questions,
  ...
  // Flush client answers to DB so they're persisted even if autosave hasn't fired yet
  if (clientAnswers) {
    await admin
      .from("assessments")
      .update({
  ...
  if (planErr || !plan) {
    console.error("Failed to create plan row:", planErr);
    return NextResponse.json(
  ...
  // Build prompt context — use merged answers (not raw DB values)
  ...
  // Build available content catalog for Claude (prevents hallucinated slugs)
...
Personalize the archetype's action playbook to this member's discipline, creative mode, income level, constraints, and ambition. Return ONLY valid JSON.`;
  ...
  // Call Claude API (async — don't block the response)
  generateRoadmap(plan.id, userPrompt, admin).catch((err) => {
...
async function generateRoadmap(
  ...
  if (!apiKey) {
    await admin
      .from("strategic_plans")
      .update({ status: "draft", plan_content: { error: "No API key configured" } })
      .eq("id", planId);
    return;
  }
  ...
  try {
    ...
    try {
      ...
      // Try to extract JSON from response if wrapped in code fences
    ...
    // Determine status based on auto-release config
