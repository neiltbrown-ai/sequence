...
import { SearchIcon } from "@/components/portal/icons";
...
interface AdminSearchResult {
  ...
  label: string;
  sublabel: string;
  type: "member" | "discount_code" | "university_code";
  href: string;
}
...
export default function AdminTopbar() {
  ...
  function handleSearchFocus() {
    ...
    if (results.length > 0 && searchQuery.length >= 2) setSearchOpen(true);
  }
  ...
  const typeLabel = (t: string) => {
    switch (t) {
      ...
      case "university_code": return "University Code";
      default: return t;
    }
  };
    ...
    <header className="topbar">
      ...
      <div className="tb-search">
        ...
        <input
          ...
          placeholder="Search members, codes…"
          value={searchQuery}
        ...
        {searchOpen && (
          <div className="tb-search-results">
            {results.length === 0 ? (
              <div className="tb-sr-empty">
                {searching ? "Searching…" : `No results for \u201c${searchQuery}\u201d`}
              </div>
            ) : (
              Object.entries(grouped).map(([type, items]) => (
                <div key={type} className="tb-sr-group">
                  <div className="tb-sr-label">{typeLabel(type)}s</div>
                  {items.map((r) => (
                    <Link
                      key={r.id}
                      href={r.href}
                    ...
                    >
                      <span className="tb-sr-title">{r.label}</span>
                      <span className="tb-sr-excerpt">{r.sublabel}</span>
                    </Link>
                  ))}
                </div>
              ))
      ...
      <div className="tb-right">
        <div className="tb-avatar-wrap" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
          <button type="button" className="tb-avatar">
            {initials}
          </button>
          {menuOpen && (
